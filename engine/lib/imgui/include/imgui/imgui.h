...
// - Gallery               https://github.com/ocornut/imgui/issues/5886 (please post your
...
// (imgui_impl_xxx.h) Using dear imgui via a shared library is not recommended, because we don't
// guarantee backward nor forward ABI compatibility (also function call overhead, as dear imgui is a
// call-heavy API)
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas
...
typedef int ImGuiSortDirection;  // -> enum ImGuiSortDirection_   // Enum: A sorting direction
...
// Flags (declared as int for compatibility with old C++, to allow using as flags without overhead,
// and to not pollute the top of this file)
// - Tip: Use your programming IDE navigation facilities on the names in the _central column_ below
// to find the actual flags/enum lists!
//   In Visual Studio IDE: CTRL+comma ("Edit.GoToAll") can follow symbols in comments, whereas
//   CTRL+F12 ("Edit.GoToImplementation") cannot. With Visual Assist installed: ALT+G
...
typedef int ImGuiInputTextFlags;  // -> enum ImGuiInputTextFlags_  // Flags: for InputText(),
                                  // InputTextMultiline()
typedef int ImGuiKeyChord;  // -> ImGuiKey | ImGuiMod_XXX    // Flags: for storage only for now: an
...
// - To use something else than an opaque void* pointer: override with e.g. '#define ImTextureID
// MyTextureType*' in your imconfig.h file.
// - This can be whatever to you want it to be! read the FAQ about ImTextureID for details.
#ifndef ImTextureID
typedef void* ImTextureID;  // Default: store a pointer or an integer fitting in a pointer (most
                            // renderer backends are ok with that)
#endif
...
// This is a frequently used type in the API. Consider using IM_VEC2_CLASS_EXTRA to create implicit
// cast from/to our preferred type.
...
//    BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed
//    in a future update.]
...
IMGUI_API void SetWindowFontScale(
    float scale);  // [OBSOLETE] set font scale. Adjust IO.FontGlobalScale if you want to scale all
...
IMGUI_API ImVec2 GetWindowContentRegionMin();  // content boundaries min for the full window
                                               // (roughly (0,0)-Scroll), in window coordinates
IMGUI_API ImVec2
GetWindowContentRegionMax();  // content boundaries max for the full window (roughly
...
IMGUI_API void PushStyleVar(ImGuiStyleVar idx,
                            const ImVec2& val);  // modify a style ImVec2 variable. always use this
                                                 // if you modify the style after NewFrame().
IMGUI_API void PopStyleVar(int count = 1);
IMGUI_API void PushTabStop(
    bool tab_stop);  // == tab stop enable. Allow focusing using TAB/Shift-TAB, enabled by default
...
IMGUI_API ImU32 GetColorU32(ImU32 col);  // retrieve given color with style alpha applied, packed as
...
IMGUI_API ImVec2
GetCursorScreenPos();  // cursor position in absolute coordinates (useful to work with ImDrawList
                       // API). generally top-left == GetMainViewport()->Pos == (0,0) in single
                       // viewport mode, and bottom-right == GetMainViewport()->Pos+Size ==
...
// ID stack/scopes
// Read the FAQ (docs/FAQ.md or http://dearimgui.org/faq) for more details about how ID are handled
// in dear imgui.
...
IMGUI_API bool BeginTooltip();  // begin/append a tooltip window. to create full-featured tooltip
...
//  afterwards. ImGuiWindowFlags are forwarded to the window.
...
// - BeginDisabled(false) essentially does nothing useful but is provided to facilitate use of
// boolean expressions. If you can avoid calling BeginDisabled(False)/EndDisabled() best to avoid
// it.
...
// - before v1.87, we used ImGuiKey to carry native/user indices as defined by each backends. About
// use of those legacy ImGuiKey values:
//  - without IMGUI_DISABLE_OBSOLETE_KEYIO (legacy support): you can still use your legacy
//  native/user indices (< 512) according to how your backend/engine stored them in io.KeysDown[],
//  but need to cast them to ImGuiKey.
//  - with    IMGUI_DISABLE_OBSOLETE_KEYIO (this is the way forward): any use of ImGuiKey will
//  assert with key < 512. GetKeyIndex() is pass-through and therefore deprecated (gone if
//  IMGUI_DISABLE_OBSOLETE_KEYIO is defined).
...
IMGUI_API const char* GetKeyName(
    ImGuiKey key);  // [DEBUG] returns English name of the key. Those names a provided for debugging
...
IMGUI_API void SetNextFrameWantCaptureMouse(
    bool want_capture_mouse);  // Override io.WantCaptureMouse flag next frame (said flag is left
                               // for your application to handle, typical when true it instucts your
                               // app to ignore inputs). This is equivalent to setting
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_UnsavedDocument =
        1 << 20,  // Display a dot next to the title. When used in a tab/docking context, tab is
                  // selected when clicking the X + closure is not assumed (will wait for user to
                  // stop submitting the tab). Otherwise closure is assumed when pressing the X, so
    ...
    ImGuiWindowFlags_NavFlattened =
        1 << 23,  // [BETA] On child window: allow gamepad/keyboard navigation to cross over parent
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_EscapeClearsAll =
        1 << 20,  // Escape key clears content if not empty, and deactivate otherwise (contrast to
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_SpanFullWidth =
        1
        << 12,  // Extend hit box to the left-most and right-most edges (bypass the indented area).
    ...
    // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 14,  // FIXME: TODO: Disable automatic scroll on
...
// - To be backward compatible with older API which took an 'int mouse_button = 1' argument, we need
// to treat
//   small flags values as a mouse button index, so we encode the mouse button in the first few bits
//   of the flags. It is therefore guaranteed to be legal to pass a mouse button index in
//   ImGuiPopupFlags.
...
enum ImGuiPopupFlags_ {
    ...
    ImGuiPopupFlags_NoOpenOverItems = 1
    ...
    ImGuiPopupFlags_AnyPopupLevel = 1 << 8,  // For IsPopupOpen(): search/test at any level of the
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument =
        1 << 0,  // Display a dot next to the title + tab is selected when clicking the X + closure
                 // is not assumed (will wait for user to stop submitting the tab). Otherwise
                 // closure is assumed when pressing the X, so if you keep submitting the tab may
    ...
    ImGuiTabItemFlags_Leading = 1 << 6,   // Enforce the tab position to the left of the tab bar
                                          // (after the tab list popup button)
    ImGuiTabItemFlags_Trailing = 1 << 7,  // Enforce the tab position to the right of the tab bar
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_RowBg =
        1 << 6,  // Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt
                 // (equivalent of calling TableSetBgColor with ImGuiTableBgFlags_RowBg0 on each row
                 // manually)
    ...
    ImGuiTableFlags_NoHostExtendY =
        1 << 17,  // Make outer height stop exactly at outer_size.y (prevent auto-extending table
    ...
    ImGuiTableFlags_PreciseWidths =
        1 << 19,  // Disable distributing remainder width to stretched columns (width allocation on
                  // a 100-wide table with 3 columns: Without this flag: 33,33,34. With this flag:
    ...
    ImGuiTableFlags_PadOuterX = 1 << 21,  // Default if BordersOuterV is on. Enable outermost
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_WidthFixed =
        1 << 4,  // Column will not stretch. Preferable with horizontal scrolling enabled (default
                 // if table sizing policy is _SizingFixedFit and table is resizable).
    ImGuiTableColumnFlags_NoResize = 1 << 5,  // Disable manual resizing.
    ImGuiTableColumnFlags_NoReorder =
        1 << 6,  // Disable manual reordering this column, this will also prevent other columns from
    ...
    ImGuiTableColumnFlags_NoHeaderLabel =
        1 << 12,  // TableHeadersRow() will not submit label for this column. Convenient for some
                  // small columns. Name will still appear in context menu.
    ...
    ImGuiTableColumnFlags_NoDirectResize_ =
        1 << 30,  // [Internal] Disable user resizing this column directly (it may however we
...
enum ImGuiFocusedFlags_ {
    ...
    ImGuiFocusedFlags_ChildWindows = 1
                                     << 0,  // Return true if any children of the window is focused
    ImGuiFocusedFlags_RootWindow =
        1 << 1,  // Test from root window (top most parent of the current hierarchy)
    ImGuiFocusedFlags_AnyWindow =
        1 << 2,  // Return true if any window is focused. Important: If you are trying to tell how
                 // to dispatch your low-level inputs, do NOT use this. Use 'io.WantCaptureMouse'
    ...
    // ImGuiFocusedFlags_DockHierarchy               = 1 << 4,   // Consider docking hierarchy
    // (treat dockspace host as parent of docked window) (when used with _ChildWindows or
    // _RootWindow)
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByPopup =
        1
        << 5,  // Return true even if a popup window is normally blocking access to this item/window
    ...
    ImGuiHoveredFlags_AllowWhenOverlapped =
        1 << 8,  // IsItemHovered() only: Return true even if the position is obstructed or
                 // overlapped by another window
    ImGuiHoveredFlags_AllowWhenDisabled =
        1 << 9,  // IsItemHovered() only: Return true even if the item is disabled
    ImGuiHoveredFlags_NoNavOverride =
        1
        << 10,  // Disable using gamepad/keyboard navigation state when active, always query mouse.
    ...
    ImGuiHoveredFlags_DelayNormal =
        1 << 11,  // Return true after io.HoverDelayNormal elapsed (~0.30 sec)
    ImGuiHoveredFlags_DelayShort =
        1 << 12,  // Return true after io.HoverDelayShort elapsed (~0.10 sec)
    ImGuiHoveredFlags_NoSharedDelay =
        1 << 13,  // Disable shared delay system where moving from one item to the next keeps the
...
// values. All our named keys are >= 512. Keys value 0 to 511 are left unused as legacy
// native/opaque key values (< 1.87). Since >= 1.89 we increased typing (went from int to enum),
// some legacy code may need a cast to ImGuiKey. Read details about the 1.87 and 1.89 transition :
// https://github.com/ocornut/imgui/issues/4921 Note that "Keys" related to physical keys and are
// not the same concept as input "Characters", the later are submitted via io.AddInputCharacter().
enum ImGuiKey : int {
    // Keyboard
    ImGuiKey_None = 0,
    ...
    ImGuiKey_GamepadFaceRight,  // B (Xbox)         A (Switch)   Circle (PS)        // Cancel /
                                // Close / Exit
    ImGuiKey_GamepadFaceUp,     // Y (Xbox)         X (Switch)   Triangle (PS)      // Text Input /
                                // On-screen Keyboard
    ImGuiKey_GamepadFaceDown,   // A (Xbox)         B (Switch)   Cross (PS)         // Activate /
                                // Open / Toggle / Tweak
    ...
    ImGuiKey_GamepadLStickRight,  // [Analog]                                         // Move Window
                                  // (in Windowing mode)
    ImGuiKey_GamepadLStickUp,     // [Analog]                                         // Move Window
                                  // (in Windowing mode)
    ...
    // index + the io.KeyMap[] array. We are ditching this method but keeping a legacy path for user
    // code doing e.g. IsKeyPressed(MY_NATIVE_KEY_CODE) If you need to iterate all keys (for e.g. an
    // input mapper) you may use ImGuiKey_NamedKey_BEGIN..ImGuiKey_NamedKey_END.
    ImGuiKey_NamedKey_BEGIN = 512,
    ImGuiKey_NamedKey_END   = ImGuiKey_COUNT,
    ImGuiKey_NamedKey_COUNT = ImGuiKey_NamedKey_END - ImGuiKey_NamedKey_BEGIN,
#ifdef IMGUI_DISABLE_OBSOLETE_KEYIO
    ImGuiKey_KeysData_SIZE   = ImGuiKey_NamedKey_COUNT,  // Size of KeysData[]: only hold named keys
    ImGuiKey_KeysData_OFFSET = ImGuiKey_NamedKey_BEGIN,  // Accesses to io.KeysData[] must use (key
    ...
    ImGuiKey_KeysData_OFFSET =
        0,  // Accesses to io.KeysData[] must use (key - ImGuiKey_KeysData_OFFSET) index.
#endif
...
// IMGUI_DISABLE_OBSOLETE_KEYIO is not set. Custom backends: feed gamepad inputs via
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableSetMousePos =
        1 << 2,  // Instruct navigation to move the mouse cursor. May be useful on TV/console
                 ...
                 // in your backend, otherwise ImGui will react as if the mouse is jumping around
                 // back and forth.
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseCursors =
        1 << 1,  // Backend Platform supports honoring GetMouseCursor() value to change the OS
                 // cursor shape.
...
// We use the same sets of flags for DragXXX() and SliderXXX() functions as the features are the
// same and it makes it easier to swap them. (Those are per-item flags. There are shared flags in
// ImGuiIO: io.ConfigDragClickToInputText)
enum ImGuiSliderFlags_ {
    ...
    ImGuiSliderFlags_Logarithmic =
        1 << 5,  // Make the widget logarithmic (linear otherwise). Consider using
                 // ImGuiSliderFlags_NoRoundToFormat with this if using a format-string with small
                 // amount of digits.
    ImGuiSliderFlags_NoRoundToFormat =
        1 << 6,  // Disable rounding underlying value to match precision of the display format
...
struct ImGuiStyle {
    ...
    float TabMinWidthForCloseButton;  // Minimum width for close button to appear on an unselected
                                      // tab when hovered. Set to 0.0f to always show when hovering,
                                      // set to FLT_MAX to never show close button unless selected.
    ImGuiDir ColorButtonPosition;     // Side of the color button in the ColorEdit4 widget
    ...
    ImVec2 DisplayWindowPadding;  // Window position are clamped to be visible within the display
                                  // area or monitors by at least this amount. Only applies to
                                  // regular windows.
    ImVec2
    ...
    bool
        AntiAliasedLines;  // Enable anti-aliased lines/borders. Disable if you are really tight on
                           // CPU/GPU. Latched at the beginning of the frame (copied to ImDrawList).
    bool AntiAliasedLinesUseTex;  // Enable anti-aliased lines/borders using textures where
                                  // possible. Require backend to render with bilinear filtering
                                  // (NOT point/nearest filtering). Latched at the beginning of the
                                  // frame (copied to ImDrawList).
...
struct ImGuiIO {
    ...
    ImVec2 DisplaySize;  // <unset>          // Main display size, in pixels (generally ==
    ...
    const char* IniFilename;     // = "imgui.ini"    // Path to .ini file (important: default
                                 // "imgui.ini" is relative to current working dir!). Set NULL to
                                 // disable automatic .ini loading/saving or if you want to manually
                                 // call LoadIniSettingsXXX() / SaveIniSettingsXXX() functions.
    ...
    float KeyRepeatRate;     // = 0.050f         // When holding a key/button, rate at which it
                             // repeats, in seconds.
    float HoverDelayNormal;  // = 0.30 sec       // Delay on hovering before
                             // IsItemHovered(ImGuiHoveredFlags_DelayNormal) returns true.
    float HoverDelayShort;   // = 0.10 sec       // Delay on hovering before
                             // IsItemHovered(ImGuiHoveredFlags_DelayShort) returns true.
    void* UserData;          // = NULL           // Store your own data.

    ImFontAtlas* Fonts;     // <auto>           // Font atlas: load, rasterize and pack one or more
    ...
    bool MouseDrawCursor;  // = false          // Request ImGui to draw a mouse cursor for you (if
                           ...
                           // renamed to 'io.ConfigXXX' because this is frequently used by backend
                           // implementations.
    bool
        ConfigMacOSXBehaviors;  // = defined(__APPLE__) // OS X style: Text editing cursor movement
    ...
    bool ConfigInputTextCursorBlink;      // = true           // Enable blinking cursor (optional as
                                          // some users consider it to be distracting).
    bool ConfigInputTextEnterKeepActive;  // = false          // [BETA] Pressing Enter will keep
                                          // item active and select contents (single-line only).
    ...
    bool ConfigWindowsMoveFromTitleBarOnly;  // = false       // Enable allowing to move windows
                                             // only when clicking on their title bar. Does not
    ...
    // - presently Begn()/End() and BeginChild()EndChild() needs to ALWAYS be called in tandem,
    // regardless of return value of BeginXXX()
    //   this is inconsistent with other BeginXXX functions and create confusion for many users.
    // - we expect to update the API eventually. In the meanwhile we provided tools to facilitate
    // checking user-code behavior.
    bool ConfigDebugBeginReturnValueOnce;  // = false         // First-time calls to
                                           // Begin()/BeginChild() will return false. NEEDS TO BE
                                           // SET AT APPLICATION BOOT TIME if you don't want to miss
                                           // windows.
    bool ConfigDebugBeginReturnValueLoop;  // = false         // Some calls to Begin()/BeginChild()
                                           // will return false. Will cycle through window depths
                                           // then repeat. Suggested use: add
                                           // "io.ConfigDebugBeginReturnValue = io.KeyShift" in your
                                           // main loop then occasionally press SHIFT. Windows
    ...
    void* BackendLanguageUserData;    // = NULL           // User data for non C++ programming
                                      // language backend
...
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    void*
        ImeWindowHandle;  // = NULL           // [Obsolete] Set ImGuiViewport::PlatformHandleRaw
                          // instead. Set this to your HWND to get automatic IME cursor positioning.
#else
    ...
    IMGUI_API void AddKeyAnalogEvent(
        ImGuiKey key,
        bool down,
        float v);  // Queue a new key down/up event for analog values (e.g. ImGuiKey_Gamepad_
                   // values). Dead-zones should be handled by the backend.
    IMGUI_API void AddMousePosEvent(
        float x,
        float y);  // Queue a mouse position update. Use -FLT_MAX,-FLT_MAX to signify no mouse (e.g.
                   // app not focused and not hovered)
    IMGUI_API void AddMouseButtonEvent(int button, bool down);  // Queue a mouse button change
    IMGUI_API void AddMouseWheelEvent(
        float wheel_x,
        float wheel_y);  // Queue a mouse wheel update. wheel_y<0: scroll down, wheel_y>0: scroll
                         // up, wheel_x<0: scroll right, wheel_x>0: scroll left.
    ...
    bool WantCaptureMouse;  // Set when Dear ImGui will use mouse inputs, in this case do not
                            // dispatch them to your main game/application (either way, always pass
                            // on mouse inputs to imgui). (e.g. unclicked mouse is hovering over an
                            // imgui window, widget is active, mouse was clicked over an imgui
                            // window, etc.).
    bool WantCaptureKeyboard;  // Set when Dear ImGui will use keyboard inputs, in this case do not
                               ...
                               // pass keyboard inputs to imgui). (e.g. InputText active, or an
                               // imgui window is focused and navigation is enabled, etc.).
    ...
    int KeyMap[ImGuiKey_COUNT];  // [LEGACY] Input: map of indices into the KeysDown[512] entries
                                 // array which represent your "native" keyboard state. The first
                                 // 512 are now unused and should be kept zero. Legacy backend will
                                 // write into KeyMap[] using ImGuiKey_ indices which are always
                                 // >512.
    bool
        KeysDown[ImGuiKey_COUNT];  // [LEGACY] Input: Keyboard keys that are pressed (ideally left
                                   // in the "native" order your engine has access to keyboard keys,
                                   // so you can use your own defines/enums for keys). This used to
                                   // be [512] sized. It is now ImGuiKey_COUNT to allow legacy
                                   // io.KeysDown[GetKeyIndex(...)] to work without an overflow.
    float NavInputs[ImGuiNavInput_COUNT];  // [LEGACY] Since 1.88, NavInputs[] was removed. Backends
                                           // from 1.60 to 1.86 won't build. Feed gamepad inputs via
                                           // io.AddKeyEvent() and ImGuiKey_GamepadXXX enums.
#endif
    ...
    float
        MouseWheelH;  // Mouse wheel Horizontal. >0 scrolls Left, <0 scrolls Right. Most users don't
    ...
    ImGuiKeyChord
        KeyMods;  // Key mods flags (any of ImGuiMod_Ctrl/ImGuiMod_Shift/ImGuiMod_Alt/ImGuiMod_Super
                  // flags, same as io.KeyCtrl/KeyShift/KeyAlt/KeySuper but merged into flags. DOES
                  // NOT CONTAINS ImGuiMod_Shortcut which is pretranslated). Read-only, updated by
                  // NewFrame()
    ImGuiKeyData KeysData[ImGuiKey_KeysData_SIZE];  // Key state for all known keys. Use IsKeyXXX()
    ...
    bool MouseClicked[5];  // Mouse button went from !Down to Down (same as MouseClickedCount[x] !=
                           // 0)
    ...
    ImU16 MouseClickedCount[5];      // == 0 (not clicked), == 1 (same as MouseClicked[]), == 2
                                     // (double-clicked), == 3 (triple-clicked) etc. when going from
                                     // !Down to Down
    ...
    bool MouseDownOwnedUnlessPopupClose[5];  // Track if button was clicked inside a dear imgui
                                             // window.
    bool MouseWheelRequestAxisSwap;  // On a non-Mac system, holding SHIFT requests WheelY to
                                     // perform the equivalent of a WheelX event. On a Mac system
                                     // this is already enforced by the system.
    float MouseDownDuration[5];  // Duration the mouse button has been down (0.0f == just clicked)
    float MouseDownDurationPrev[5];    // Previous time the mouse button has been down
    float MouseDragMaxDistanceSqr[5];  // Squared maximum distance of how much mouse has traveled
                                       // from the clicking point (used for moving thresholds)
    ...
    ImS8 BackendUsingLegacyKeyArrays;        // -1: unknown, 0: using AddKeyEvent(), 1: using legacy
                                             // io.KeysDown[]
    bool BackendUsingLegacyNavInputArray;    // 0: using AddKeyAnalogEvent(), 1: writing to legacy
                                             // io.NavInputs[] directly
...
// returns true on edit, the callback is useful mainly to manipulate the underlying buffer while
// focus is active)
...
struct ImGuiTableColumnSortSpecs {
    ...
    ImS16 SortOrder;  // Index within parent ImGuiTableSortSpecs (always stored in order starting
                      // from 0, tables sorted on a single criteria will always have a 0 here)
    ImGuiSortDirection
        SortDirection : 8;  // ImGuiSortDirection_Ascending or ImGuiSortDirection_Descending (you
...
struct ImGuiTableSortSpecs {
    ...
    bool SpecsDirty;  // Set to true when specs have changed since last time! Use this to sort
...
struct ImGuiListClipper {
    ...
    float ItemsHeight;  // [Internal] Height of item after a first step and item submission can
                        // calculate it
    float StartPosY;    // [Internal] Cursor position at the time of Begin() or after table frozen
                        // rows are all processed
    void* TempData;     // [Internal] Internal data

    // items_count: Use INT_MAX if you don't know how many items you have (in which case the cursor
    // won't be advanced in the final step) items_height: Use -1.0f to be calculated automatically
    // on first step. Otherwise pass in the distance between your items, typically
    ...
    // Call ForceDisplayRangeByIndices() before first call to Step() if you need a range of items to
    // be displayed regardless of visibility.
    IMGUI_API void ForceDisplayRangeByIndices(
        int item_min,
        int item_max);  // item_max is exclusive e.g. use (42, 42+1) to make item 42 always visible
...
// - You can use '#define IMGUI_DEFINE_MATH_OPERATORS' to import our operators, provided as a
// courtesy.
// - We unfortunately don't have a unary- operator for ImVec2 because this would needs to be defined
...
struct ImDrawCmd {
    ...
    unsigned int VtxOffset;  // 4    // Start offset in vertex buffer.
                             // ImGuiBackendFlags_RendererHasVtxOffset: always 0, otherwise may be
    ...
    // Since 1.83: returns ImTextureID associated with this draw call. Warning: DO NOT assume this
    // is always same as 'TextureId' (we will change this function for an upcoming feature)
...
enum ImDrawFlags_ {
    ImDrawFlags_None   = 0,
    ImDrawFlags_Closed = 1 << 0,  // PathStroke(), AddPolyline(): specify that shape should be
...
enum ImDrawListFlags_ {
    ...
    ImDrawListFlags_AntiAliasedLines =
        1 << 0,  // Enable anti-aliased lines/borders (*2 the number of triangles for 1.0f wide line
                 // or lines thin enough to be drawn using textures, otherwise *3 the number of
                 // triangles)
...
// transformation matrix to want to the data (depending on the use of the transformation you may
...
struct ImDrawList {
    ...
    ImDrawCmdHeader _CmdHeader;    // [Internal] template of active commands. Fields should match
    ...
    //   circles. Use AddNgon() and AddNgonFilled() functions if you need to guarantee a specific
    //   number of sides.
    ...
    // - "uv_min" and "uv_max" represent the normalized texture coordinates to use for those
    // corners. Using (0,0)->(1,1) texture coordinates will generally display the entire texture.
    ...
    // Obsolete names
    // inline  void  AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const
    // ImVec2& p4, ImU32 col, float thickness, int num_segments = 0) { AddBezierCubic(p1, p2, p3,
    // p4, col, thickness, num_segments); } // OBSOLETED in 1.80 (Jan 2021) inline  void
...
struct ImDrawData {
    ...
    ImVec2 DisplayPos;      // Top-left position of the viewport to render (== top-left of the
...
struct ImFontConfig {
    ...
    const ImWchar* GlyphRanges;  // NULL     // THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT
                                 // IS ALIVE. Pointer to a user-provided list of Unicode range (2
...
enum ImFontAtlasFlags_ {
    ...
    ImFontAtlasFlags_NoBakedLines =
        1 << 2,  // Don't build thick line textures into the atlas (save a little texture memory,
                 // allow support for point/nearest filtering). The AntiAliasedLinesUseTex features
                 // uses them, otherwise they will be rendered using polygons (more expensive for
                 // CPU/GPU).
};
...
struct ImFontAtlas {
    ...
    }  // Bit ambiguous: used to detect when user didn't build texture but effectively we should
       // check TexID != 0 except that would be backend dependent...
    ...
    int TexGlyphPadding;  // Padding between glyphs within texture in pixels. Defaults to 1. If your
                          ...
                          // 0 (will also need to set AntiAliasedLinesUseTex = false).
    bool Locked;  // Marked as Locked by ImGui::NewFrame() so attempt to modify the atlas will
                  // assert.
    void*
        UserData;  // Store your own atlas related user-data (if e.g. you have multiple font atlas).
...
struct ImFont {
    ...
    ImU8
        Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
                       8];  // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_OwnedByApp        = 1 << 2,  // Platform Window: is created/managed by the
                                             // application (rather than a dear imgui backend)
};
...
struct ImGuiViewport {
    ...
    void* PlatformHandleRaw;  // void* to hold lower-level, platform-native window handle (under
                              // Win32 this is expected to be a HWND, unused for other platforms)
...
IMGUI_API bool ImageButton(
    ...
    int frame_padding      = -1,
    const ImVec4& bg_col   = ImVec4(0, 0, 0, 0),
    const ImVec4& tint_col = ImVec4(1, 1, 1, 1));  // Use new ImageButton() signature (explicit item
                                                   // id, regular FramePadding)
...
enum ImDrawCornerFlags_ {
    ImDrawCornerFlags_None = ImDrawFlags_RoundCornersNone,  // Was == 0 prior to 1.82, this is now
                                                            // == ImDrawFlags_RoundCornersNone which
                                                            // is != 0 and not implicit
...
typedef ImGuiKeyChord
    ImGuiModFlags;  // == int. We generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any
