...
// guarantee of forward compatibility. To implement maths operators for ImVec2 (disabled by default
// to not conflict with using IM_VEC2_CLASS_EXTRA with your own math types+operators), use:
/*
...
#pragma clang diagnostic ignored \
    "-Wfloat-equal"  // warning: comparing floating point with == or != is unsafe // storing and
                     // comparing against same constants ok, for ImFloorSigned()
...
typedef int ImGuiInputFlags;     // -> enum ImGuiInputFlags_         // Flags: for IsKeyPressed(),
                                 // IsMouseClicked(), SetKeyOwner(), SetItemKeyOwner() etc.
...
IMGUI_API ImVec2
ImBezierCubicClosestPoint(const ImVec2& p1,
                          const ImVec2& p2,
                          const ImVec2& p3,
                          const ImVec2& p4,
                          const ImVec2& p,
...
struct ImGuiTextIndex {
    ...
    int EndOffset = 0;  // Because we don't own text buffer we need to maintain EndOffset (may bake
                        // in LineOffsets?)
...
// You may want to create your own instance of this if you want to use ImDrawList completely without
// ImGui. In that case, watch out for future changes to this structure.
...
enum ImGuiItemFlags_ {
    ...
    ImGuiItemFlags_NoTabStop = 1 << 0,  // false     // Disable keyboard tabbing. This is a
                                        // "lighter" version of ImGuiItemFlags_NoNav.
    ImGuiItemFlags_ButtonRepeat =
        1 << 1,  // false     // Button() will return true multiple times based on io.KeyRepeatDelay
                 // and io.KeyRepeatRate settings.
    ImGuiItemFlags_Disabled =
        1 << 2,  // false     // Disable interactions but doesn't affect visuals. See
                 // BeginDisabled()/EndDisabled(). See github.com/ocornut/imgui/issues/211
    ImGuiItemFlags_NoNav = 1 << 3,  // false     // Disable any form of focusing (keyboard/gamepad
                                    // directional navigation and SetKeyboardFocusHere() calls)
    ImGuiItemFlags_NoNavDefaultFocus = 1 << 4,  // false     // Disable item being a candidate for
                                                // default focus (e.g. used by title bar items)
    ImGuiItemFlags_SelectableDontClosePopup = 1 << 5,  // false     // Disable MenuItem/Selectable()
                                                       // automatically closing their popup window
    ...
    ImGuiItemFlags_ReadOnly = 1 << 7,  // false     // [ALPHA] Allow hovering interactions but
    ...
    ImGuiItemFlags_Inputable =
        1 << 10,  // false     // [WIP] Auto-activate input mode when tab focused. Currently only
...
enum ImGuiItemStatusFlags_ {
    ...
    ImGuiItemStatusFlags_HasDeactivated = 1
                                          << 5,  // Set if the widget/group is able to provide data
    ...
    ImGuiItemStatusFlags_FocusedByTabbing = 1 << 8,  // Set when the Focusable item just got focused
                                                     // by Tabbing (FIXME: to be removed soon)
    ImGuiItemStatusFlags_Visible = 1
                                   << 9,  // [WIP] Set when item is overlapping the current clipping
                                          // rectangle (Used internally. Please don't use yet:
                                          // API/system will change as we refactor Itemadd()).
...
enum ImGuiButtonFlagsPrivate_ {
    ...
    // ImGuiButtonFlags_Disabled             = 1 << 14,  // disable interactions -> use
...
enum ImGuiSelectableFlagsPrivate_ {
    ...
    ImGuiSelectableFlags_SelectOnNav =
        1 << 21,  // (WIP) Auto-select when moved into. This is not exposed in public API as to
                  // handle multi-select and modifiers we will need user to explicitly control focus
                  // scope. May be replaced with a BeginSelection() API.
...
struct ImGuiPopupData {
    ...
    ImGuiWindow*
        BackupNavWindow;   // Set on OpenPopup(), a NavWindow that will be restored on popup close
...
struct ImGuiKeyRoutingData {
    ...
    ImU16 Mods;  // Technically we'd only need 4-bits but for simplify we store ImGuiMod_ values
                 // which need 16-bits. ImGuiMod_Shortcut is already translated to Ctrl/Super.
...
// SetItemKeyOwner() Don't mistake with ImGuiInputTextFlags! (for ImGui::InputText() function)
enum ImGuiInputFlags_ {
    // Flags for IsKeyPressed(), IsMouseClicked(), Shortcut()
    ImGuiInputFlags_None = 0,
    ImGuiInputFlags_Repeat =
        1 << 0,  // Return true on successive repeats. Default for legacy IsKeyPressed(). NOT
    ...
    ImGuiInputFlags_LockThisFrame =
        1 << 6,  // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT
                 // accepted for polling). Cleared at end of frame. This is useful to make
                 // input-owner-aware code steal keys from non-input-owner-aware code.
    ImGuiInputFlags_LockUntilRelease =
        1 << 7,  // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0 will NOT
                 // accepted for polling). Cleared when the key is released or at end of each frame
                 // if key is released. This is useful to make input-owner-aware code steal keys
                 // from non-input-owner-aware code.

    // Routing policies for Shortcut() + low-level SetShortcutRouting()
    // - The general idea is that several callers register interest in a shortcut, and only one
    // owner gets it.
    // - When a policy (other than _RouteAlways) is set, Shortcut() will register itself with
    // SetShortcutRouting(),
    //   allowing the system to decide where to route the input among other route-aware calls.
    // - Shortcut() uses ImGuiInputFlags_RouteFocused by default: meaning that a simple Shortcut()
    // poll
    //   will register a route and only succeed when parent window is in the focus stack and if
    //   no-one with a higher priority is claiming the shortcut.
    // - Using ImGuiInputFlags_RouteAlways is roughly equivalent to doing e.g. IsKeyPressed(key) +
    // testing mods.
    // - Priorities: GlobalHigh > Focused (when owner is active item) > Global > Focused (when
    // focused window) > GlobalLow.
    // - Can select only 1 policy among all available.
    ImGuiInputFlags_RouteFocused =
        1 << 8,  // (Default) Register focused route: Accept inputs if window is in focus stack.
                 // Deep-most focused window takes inputs. ActiveId takes inputs over deep-most
                 // focused window.
    ImGuiInputFlags_RouteGlobalLow =
        1 << 9,  // Register route globally (lowest priority: unless a focused window or active item
                 // registered the route) -> recommended Global priority.
    ImGuiInputFlags_RouteGlobal =
        1 << 10,  // Register route globally (medium priority: unless an active item registered the
                  // route, e.g. CTRL+A registered by InputText).
    ImGuiInputFlags_RouteGlobalHigh =
        1 << 11,  // Register route globally (highest priority: unlikely you need to use that: will
                  // interfere with every active items)
    ...
    ImGuiInputFlags_RouteUnlessBgFocused =
        1 << 13,  // Global routes will not be applied if underlying background/void is focused (==
                  // no Dear ImGui windows are focused). Useful for overlay applications.
...
enum ImGuiActivateFlags_ {
    ...
    ImGuiActivateFlags_TryToPreserveState =
        1 << 2,  // Request widget to preserve state if it can (e.g. InputText will try to preserve
                 // cursor/selection)
};
...
enum ImGuiScrollFlags_ {
    ...
    ImGuiScrollFlags_KeepVisibleEdgeX =
        1 << 0,  // If item is not visible: scroll as little as possible on X axis to bring item
                 // back into view [default for X axis]
    ImGuiScrollFlags_KeepVisibleEdgeY =
        1 << 1,  // If item is not visible: scroll as little as possible on Y axis to bring item
                 // back into view [default for Y axis for windows that are already visible]
    ...
    ImGuiScrollFlags_AlwaysCenterX = 1
                                     << 4,  // Always center the result item on X axis [rarely used]
    ImGuiScrollFlags_AlwaysCenterY =
        1
        << 5,  // Always center the result item on Y axis [default for Y axis for appearing window)
...
enum ImGuiNavMoveFlags_ {
    ...
    ImGuiNavMoveFlags_DebugNoResult = 1
                                      << 8,  // Dummy scoring for debug purpose, don't apply result
...
enum ImGuiOldColumnFlags_ {
    ...
    ImGuiOldColumnFlags_NoResize = 1
    ...
    ImGuiOldColumnFlags_NoForceWithinWindow = 1
...
struct ImGuiViewportP : public ImGuiViewport {
    int DrawListsLastFrame[2];  // Last frame number the background (0) and foreground (1) draw
                                // lists were used
    ...
    ImVec2 WorkOffsetMin;  // Work Area: Offset from Pos to top-left corner of Work Area. Generally
    ...
    ImVec2 WorkOffsetMax;  // Work Area: Offset from Pos+Size to bottom-right corner of Work Area.
                           // Generally (0,0) or (0,-status_bar_height).
    ImVec2 BuildWorkOffsetMin;  // Work Area: Offset being built during current frame. Generally >=
                                // 0.0f.
    ImVec2 BuildWorkOffsetMax;  // Work Area: Offset being built during current frame. Generally <=
                                // 0.0f.
...
struct ImGuiStackLevelInfo {
    ...
    char Desc[57];  // Arbitrarily sized buffer to hold a result (FIXME: could replace Results[]
                    // with a chunk stream?) FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext {
    ...
    ImVec2 WindowsHoverPadding;  // Padding around resizable windows for which hovering on counts as
                                 // hovering the window == ImMax(style.TouchExtraPadding,
                                 // WINDOWS_HOVER_PADDING)
    ImGuiWindow* CurrentWindow;  // Window being drawn into
    ImGuiWindow* HoveredWindow;  // Window the mouse is hovering. Will typically catch mouse inputs.
    ImGuiWindow* HoveredWindowUnderMovingWindow;  // Hovered window ignoring MovingWindow. Only set
    ...
    ImGuiID
        DebugHookIdInfo;  // Will call core hooks: DebugHookIdInfo() from GetID functions, used by
                          // Stack Tool [next HoveredId/ActiveId to not pull in an extra cache-line]
    ...
    // - Input query can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None
    // (== -1) or a custom ID.
    // - Routing is requested ahead of time for a given chord (Key + Mods) and granted in
    // NewFrame().
    ...
    bool ActiveIdUsingAllKeyboardKeys;  // Active widget will want to read all keyboard keys inputs.
                                        // (FIXME: This is a shortcut for not taking ownership of
                                        // 100+ keys but perhaps best to not have the inconsistency)
#ifndef IMGUI_DISABLE_OBSOLETE_KEYIO
    ImU32
        ActiveIdUsingNavInputMask;  // If you used this. Since (IMGUI_VERSION_NUM >= 18804) :
    ...
    ImGuiID DebugLocateId;  // Storage for DebugLocateItemOnHover() feature: this is read by
                            // ItemAdd() so we keep it in a hot/cached location
    ...
    ImVector<ImGuiID> FocusScopeStack;  // Stack for PushFocusScope()/PopFocusScope() - inherited by
    ...
    ImGuiID NavFocusScopeId;  // Identify a selection scope (selection code often wants to "clear
    ...
    ImGuiID NavInitResultId;      // Init request result (first item of the window, or one for which
    ...
    ImGuiNavItemData NavMoveResultLocalVisible;  // Best move request candidate within NavWindow
    ...
    // move/resize)
    ImGuiKeyChord ConfigNavWindowingKeyNext;  // = ImGuiMod_Ctrl | ImGuiKey_Tab, for reconfiguration
                                              // (see #4828)
    ...
    // Drag and Drop
    bool DragDropActive;
    bool DragDropWithinSource;  // Set when within a BeginDragDropXXX/EndDragDropXXX block for a
                                // drag source.
    bool DragDropWithinTarget;  // Set when within a BeginDragDropXXX/EndDragDropXXX block for a
                                // drag target.
    ...
    float HoverDelayTimer;       // Currently used IsItemHovered(), generally inferred from
                                 // g.HoveredIdTimer but kept uncleared until clear timer elapse.
    float HoverDelayClearTimer;  // Currently used IsItemHovered(): grace time before
    ...
    float ColorEditSavedHue;    // Backup of last Hue associated to LastColor, so we can restore Hue
                                // in lossy RGB<>HSV round trips
    float ColorEditSavedSat;    // Backup of last Saturation associated to LastColor, so we can
    ...
    ImGuiPlatformImeData PlatformImeDataPrev;  // Previous frame data (when changing we will call
    ...
    ImU8 DebugLocateFrames;  // For DebugLocateItemOnHover(). This is used together with
                             // DebugLocateId which is in a hot/cached spot above.
...
struct IMGUI_API ImGuiWindowTempData {
    ...
    ImVec2 CursorStartPosLossyness;  // Record the loss of precision of CursorStartPos due to really
                                     // large scrolling amount. This is used by clipper to
                                     // compensentate and fix the most common use case of large
                                     // scroll area.
    ...
    ImU32 TreeJumpToParentOnPopMask;  // Store a copy of !g.NavIdIsAlive for TreeDepth 0..31.. Could
                                      // be turned into a ImU64 if necessary.
...
struct IMGUI_API ImGuiWindow {
    ...
    float DecoInnerSizeX1,
        DecoInnerSizeY1;  // Applied AFTER/OVER InnerRect. Specialized for Tables as they use
                          // specialized form of clipping and frozen rows/columns are inside
                          // InnerRect (and not part of regular decoration sizes).
    ...
    short BeginOrderWithinParent;   // Begin() order within immediate parent window, if we are a
                                    // child window. Otherwise 0.
    short BeginOrderWithinContext;  // Begin() order within entire imgui context. This is mostly
    ...
    ImVec2 SetWindowPosPivot;  // store window pivot for positioning. ImVec2(0, 0) when positioning
                               // from top-left corner; ImVec2(0.5f, 0.5f) for centering; ImVec2(1,
                               // 1) for bottom right.
    ...
    ImRect ParentWorkRect;  // Backup of WorkRect before entering a container such as
    ...
    float LastTimeActive;  // Last timestamp the window was Active (using float as we don't need
                           // high precision there)
...
enum ImGuiTabBarFlagsPrivate_ {
    ...
    ImGuiTabBarFlags_SaveSettings =
        1 << 22,  // FIXME: Settings are handled by the docking system, this only request the tab
                  // bar to mark settings dirty when reordering tabs
};
...
struct ImGuiTableColumn {
    ...
    float WidthGiven;             // Final/actual width visible == (MaxX - MinX), locked in
                       // TableUpdateLayout(). May be > WidthRequest to honor minimum width, may be
                       // < WidthRequest to honor shrinking columns down in tight space.
    ...
    float StretchWeight;  // Master width weight when (Flags & _WidthStretch). Often around ~1.0f
                          // initially.
    float InitStretchWeightOrWidth;  // Value passed to TableSetupColumn(). For Width it is a
                                     // content width (_without padding_).
    ...
    float ContentMaxXFrozen;  // Contents maximum position for frozen rows (apart from headers),
                              // from which we can infer content width.
    float ContentMaxXUnfrozen;
    float
        ContentMaxXHeadersUsed;  // Contents maximum position for headers rows (regardless of
    ...
    ImGuiTableColumnIdx PrevEnabledColumn;  // Index of prev enabled/visible column within
                                            // Columns[], -1 if first enabled/visible column
    ImGuiTableColumnIdx NextEnabledColumn;  // Index of next enabled/visible column within
    ...
    bool IsUserEnabled;  // Is the column not marked Hidden by the user? (unrelated to being off
                         // view, e.g. clipped by scrolling).
    ...
    ImU8 CannotSkipItemsQueue;  // Queue of 8 values for the next 8 frames to disable
                                // Clipped/SkipItem
...
struct ImGuiTableInstanceData {
    ...
    float LastFirstRowHeight;  // Height of first row from last frame (FIXME: this is used as
                               // "header height" and may be reworked)
...
struct IMGUI_API ImGuiTable {
    ...
    ImBitArrayPtr
        EnabledMaskByIndex;  // Column Index -> IsEnabled map (== not hidden by user/api) in a
                             // format adequate for iterating column without touching cold data
    ImBitArrayPtr VisibleMaskByIndex;  // Column Index -> IsVisibleX|IsVisibleY map (== not hidden
                                       // by user/api && not hidden by scrolling/cliprect)
    ...
    float ColumnsAutoFitWidth;  // Sum of ideal column width in order nothing to be clipped, used
                                // for auto-fitting and content width submission in outer window
    ...
    ImRect BgClipRect;  // We use this to cpu-clip cell background color fill, evolve during the
                        // frame as we cross frozen rows boundaries
    ...
    ImGuiTableColumnIdx ContextPopupColumn;  // Column right-clicked on, of -1 if opening context
    ...
    ImGuiTableColumnIdx
        FreezeColumnsCount;  // Actual frozen columns count (== FreezeColumnsRequest, or == 0 when
                             // no scrolling offset)
    ...
    bool
        IsLayoutLocked;  // Set by TableUpdateLayout() which is called when beginning the first row.
    ...
    bool
        IsDefaultSizingPolicy;  // Set if user didn't explicitly set a sizing policy in BeginTable()
    bool HasScrollbarYCurr;  // Whether ANY instance of this table had a vertical scrollbar during
                             // the current frame.
    bool HasScrollbarYPrev;  // Whether ANY instance of this table had a vertical scrollbar during
                             // the previous.
...
struct IMGUI_API ImGuiTableTempData {
    ...
    ImRect HostBackupParentWorkRect;  // Backup of InnerWindow->ParentWorkRect at the end of
                                      // BeginTable()
    ImVec2 HostBackupPrevLineSize;    // Backup of InnerWindow->DC.PrevLineSize at the end of
                                      // BeginTable()
    ImVec2 HostBackupCurrLineSize;    // Backup of InnerWindow->DC.CurrLineSize at the end of
                                      // BeginTable()
    ImVec2 HostBackupCursorMaxPos;    // Backup of InnerWindow->DC.CursorMaxPos at the end of
                                      // BeginTable()
    ImVec1 HostBackupColumnsOffset;   // Backup of OuterWindow->DC.ColumnsOffset at the end of
                                      // BeginTable()
    float HostBackupItemWidth;  // Backup of OuterWindow->DC.ItemWidth at the end of BeginTable()
    int HostBackupItemWidthStackSize;  // Backup of OuterWindow->DC.ItemWidthStack.Size at the end
                                       // of BeginTable()
...
struct ImGuiTableSettings {
    ...
    ImGuiTableFlags SaveFlags;  // Indicate data we want to save using the
                                // Resizable/Reorderable/Sortable/Hideable flags (could be using its
                                // own flags..)
    float RefScale;  // Reference scale to be able to rescale columns on font/dpi changes.
...
inline ImGuiKeyChord ConvertShortcutMod(ImGuiKeyChord key_chord) {
    ...
    return (key_chord & ~ImGuiMod_Shortcut) |
           (g.IO.ConfigMacOSXBehaviors ? ImGuiMod_Super : ImGuiMod_Ctrl);
}
...
// - Input queries can then read input by specifying ImGuiKeyOwner_Any (== 0), ImGuiKeyOwner_None
// (== -1) or a custom ID.
...
IMGUI_API void SetItemKeyOwner(
    ...
    ImGuiInputFlags flags =
        0);  // Set key owner to last item if it is hovered or active. Equivalent to 'if
...
//   ONLY ImGuiMod_XXX values are legal to 'OR' with an ImGuiKey. You CANNOT 'OR' two ImGuiKey
//   values.
// - When using one of the routing flags (e.g. ImGuiInputFlags_RouteFocused): routes requested ahead
// of time given a chord (key + modifiers) and a routing policy.
...
// - Route is granted to a single owner. When multiple requests are made we have policies to select
// the winning route.
// - Multiple read sites may use the same owner id and will all get the granted route.
// - For routing: when owner_id is 0 we use the current Focus Scope ID as a default owner in order
// to identify our location.
IMGUI_API bool Shortcut(ImGuiKeyChord key_chord, ImGuiID owner_id = 0, ImGuiInputFlags flags = 0);
IMGUI_API bool SetShortcutRouting(ImGuiKeyChord key_chord,
                                  ImGuiID owner_id      = 0,
                                  ImGuiInputFlags flags = 0);
...
IMGUI_API int
TableGetHoveredColumn();  // May use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered)
                          // instead. Return hovered column. return -1 when table is not hovered.
                          // return columns_count if the unused space at the right of visible
                          // columns is hovered.
...
IMGUI_API void DebugNodeWindowsListByBeginStackParent(ImGuiWindow** windows,
                                                      int windows_size,
...
// widgets that used FocusableItemRegister():
...
//  ImGuiItemAddFlags_Focusable)'  and 'bool tab_focused = (GetItemStatusFlags() &
//  ImGuiItemStatusFlags_Focused) != 0' (New) IMGUI_VERSION_NUM >= 18413: using 'ItemAdd(...,
//  ImGuiItemFlags_Inputable)'     and 'bool tab_focused = (GetItemStatusFlags() &
...
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
    if (g.TestEngineHookItems)                           \
    ImGuiTestEngineHook_ItemInfo(&g,                     \
                                 _ID,                    \
                                 _LABEL,                 \
