...
bool ImGui::ButtonBehavior(const ImRect& bb,
    ...
    if (hovered) {
        // Poll mouse buttons
        // - 'mouse_button_clicked' is generally carried into ActiveIdMouseButton when setting
        // ActiveId.
        // - Technically we only need some values in one code path, but since this is gated by
        // hovered test this is fine.
    ...
    if (g.NavActivateDownId == id) {
        ...
        if (!nav_activated_by_inputs && (flags & ImGuiButtonFlags_Repeat)) {
            ...
            const float t1 =
                ImMax(ImMax(key1->DownDuration, key2->DownDuration), key3->DownDuration);
            nav_activated_by_inputs = CalcTypematicRepeatAmount(t1 - g.IO.DeltaTime,
                                                                t1,
                                                                g.IO.KeyRepeatDelay,
...
void ImGui::Image(ImTextureID user_texture_id,
    ...
    if (border_col.w > 0.0f) {
        ...
        window->DrawList
            ->AddImage(user_texture_id, bb.Min, bb.Max, uv0, uv1, GetColorU32(tint_col));
    }
}
...
// - new ImageButton() requires an explicit 'const char* str_id'    Old ImageButton() used opaque
// imTextureId (created issue with: multiple buttons with same image, transient texture id values,
// opaque computation of ID)
// - new ImageButton() always use style.FramePadding                Old ImageButton() had an
// override argument. If you need to change padding with new ImageButton() you can use
// PushStyleVar(ImGuiStyleVar_FramePadding, value), consistent with other Button functions.
...
bool ImGui::RadioButton(const char* label, bool active) {
    ...
    if (style.FrameBorderSize > 0.0f) {
        window->DrawList->AddCircle(center + ImVec2(1, 1),
                                    radius,
...
// FIXME: Surprisingly, this seemingly simple widget is adjacent to MANY different legacy/tricky
// layout issues.
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags) {
    ...
    if (flags & ImGuiSeparatorFlags_Vertical) {
        ...
        // FIXME-WORKRECT: In theory we should simply be using WorkRect.Min.x/Max.x everywhere but
        // it isn't aesthetically what we want, need to introduce a variant of WorkRect for that
        // purpose. (#4787)
        ...
        const float thickness_for_layout =
            (thickness == 1.0f)
                ? 0.0f
                : thickness;  // FIXME: See 1.70/1.71 Separator() change: makes legacy 1-px
...
void ImGui::Separator() {
    ...
    flags |= ImGuiSeparatorFlags_SpanAllColumns;  // NB: this only applies to legacy Columns() api
                                                  // as they relied on Separator() a lot.
    SeparatorEx(flags);
}
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w) {
    ...
    const float text_baseline_y = ImFloor(
...
void ImGui::SeparatorText(const char* label) {
    ...
    // The SeparatorText() vs SeparatorTextEx() distinction is designed to be considerate that we
    // may want:
    // - allow headers to be draggable items (would require a stable ID + a noticeable highlight)
    // - this high-level entry point to allow formatting? (may require ID separate from formatted
    // string)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags) {
    ...
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x, bb.Min.y + style.FramePadding.y),
                   label);
...
bool ImGui::DataTypeApplyFromText(const char* buf,
    ...
    if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
        ...
        format = ImParseFormatSanitizeForScanning(format,
                                                  format_sanitized,
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type,
    ...
    if (is_logarithmic) {
        ...
        float v_old_parametric =
            ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
                                                              v_cur,
                                                              v_min,
                                                              v_max,
                                                              is_logarithmic,
...
bool ImGui::DragScalar(const char* label,
    ...
    if (!temp_input_is_active) {
        ...
        const bool make_active =
            (input_requested_by_tabbing || clicked || double_clicked || g.NavActivateId == id);
...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type,
    ...
    if (is_logarithmic) {
        ...
        if ((v_min * v_max) < 0.0f)  // Range crosses zero, so we have to do this in two parts
        {
            ...
            result =
                (TYPE)(v_min_fudged * ImPow(v_max_fudged / v_min_fudged, (FLOATTYPE)t_with_flip));
    } else {
        // Linear slider
        const bool is_floating_point =
            (data_type == ImGuiDataType_Float) || (data_type == ImGuiDataType_Double);
...
bool ImGui::SliderBehaviorT(const ImRect& bb,
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            float input_delta = (axis == ImGuiAxis_X) ? GetNavTweakPressedAmount(axis)
            ...
            if (g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated) {
                                                          ...
                                                          // saturation when already past the limits
                {
                    ...
                    TYPE v_new =
                        ScaleValueFromRatioT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type,
                                                                          clicked_t,
                                                                          v_min,
                                                                          v_max,
                                                                          is_logarithmic,
...
bool ImGui::SliderBehavior(const ImRect& bb,
    ...
    // Those are the things we can do easily outside the SliderBehaviorT<> template, saves code
    // generation.
    ImGuiContext& g = *GImGui;
    if ((g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) || (flags & ImGuiSliderFlags_ReadOnly))
        return false;
...
static void STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r,
    ...
    const ImVec2 size             = InputTextCalcTextSizeW(obj->Ctx,
                                               text + line_start_idx,
                                               text + obj->CurLenW,
                                               &text_remaining,
                                               NULL,
...
static int is_word_boundary_from_right(ImGuiInputTextState* obj, int idx) {
    // When ImGuiInputTextFlags_Password is set, we don't want actions such as CTRL+Arrow to leak
...
static bool InputTextFilterCharacter(unsigned int* p_char,
    ...
    if (c < 0x20) {
        ...
        pass |= (c == '\n' &&
                 (flags &
                  ImGuiInputTextFlags_Multiline));  // Note that an Enter KEY will emit \r and be
    ...
    if (apply_named_filters &&
        ...
        // which affect the output/input of printf/scanf to use e.g. ',' instead of '.'. The
        // standard mandate that programs starts in the "C" locale where the decimal point is '.'.
        // We don't really intend to provide widespread support for it, but out of empathy for
        // people stuck with using odd API, we support the bare minimum aka overriding the decimal
        ...
        // (https://en.wikipedia.org/wiki/Halfwidth_and_Fullwidth_Forms_(Unicode_block) While this
        // is mostly convenient, this has the side-effect for uninformed users accidentally
        // inputting full-width characters that they may scratch their head as to why it works in
        // numerical fields vs in generic text fields it would require support in the font.
...
// Important: needs to be run before TextW is rewritten with the new characters because calling
// STB_TEXTEDIT_GETCHAR() at the end.
...
// buffer directly.
static void InputTextReconcileUndoStateAfterUserCallback(ImGuiInputTextState* state,
...
bool ImGui::InputTextEx(const char* label,
    ...
    if (is_multiline) {
        ...
        // FIXME-NAV: Pressing NavActivate will trigger general child activation right before
        // triggering our own below. Harmless but bizarre.
    ...
    const bool input_requested_by_tabbing =
        (item_status_flags & ImGuiItemStatusFlags_FocusedByTabbing) != 0;
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs) {
        ...
        // on the same frame they report IsItemDeactivatedAfterEdit (#4714)
        ...
        // FIXME: Since we reworked this on 2022/06, may want to differenciate recycle_cursor vs
        // recycle_undostate?
    ...
    if (g.ActiveId == id) {
        ...
        if (select_all) {
            ...
            if ((multiclick_count % 2) == 0) {
                ...
                // We always use the "Mac" word advance for double-click select vs CTRL+Right which
                // use the platform dependent variant:
                // FIXME: There are likely many ways to improve this behavior, but there's no
                // "right" behavior (depends on use-case, software, OS)
                const bool is_bol =
                    (state->Stb.cursor == 0) ||
        ...
        // (#2467, #1336) (For Tab and Enter: Win32/SFML/Allegro are sending both keys and chars,
        // GLFW and SDL are only sending keys. For Space they all send all threes)
        if ((flags & ImGuiInputTextFlags_AllowTabInput) && Shortcut(ImGuiKey_Tab, id) &&
            !is_readonly) {
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        // operations for other code (e.g. calling window trying to use CTRL+A and CTRL+B: formet
        // would be handled by InputText) Otherwise we could simply assume that we own the keys as
        // we are active.
        ...
        // We allow validate/cancel with Nav source (gamepad) to makes it easier to undo an
        // accidental NavInput press with no keyboard wired, but otherwise it isn't very useful.
        ...
        if (IsKeyPressed(ImGuiKey_LeftArrow)) {
            ...
            if (!state->HasSelection()) {
                // OSX doesn't seem to have Super+Delete to delete until end-of-line, so we don't
    ...
    if (g.ActiveId == id) {
        ...
        if (revert_edit && !is_readonly) {
            if (flags & ImGuiInputTextFlags_EscapeClearsAll) {
                ...
                // the current buffer contents. Push records into the undo stack so we can CTRL+Z
                // the revert operation itself
                ...
                if (event_flag) {
                    ...
                    callback_buf =
                        is_readonly
                            ? buf
                            : state->TextA
                                  .Data;  // Pointer may have been invalidated by a resize callback
                    ...
                    if (buf_dirty) {
                        ...
                        InputTextReconcileUndoStateAfterUserCallback(
                            ...
                            callback_data.Buf,
                            callback_data
                                .BufTextLen);  // FIXME: Move the rest of this block inside function
                                               // and rename to
                        ...
                        state->CurLenW = ImTextStrFromUtf8(state->TextW.Data,
                                                           state->TextW.Size,
                                                           callback_data.Buf,
                                                           NULL);
                        state->CurLenA =
    ...
    if (render_cursor || render_selection) {
...

        {
            ...
            cursor_offset.x = InputTextCalcTextSizeW(&g,
    ...
    if (is_multiline) {
        ...
        // specifies the ImGuiItemFlags_Inputable (ref issue #4761)...
        ...
        // ...and then we need to undo the group overriding last item data, which gets a bit messy
        // as EndGroup() tries to forward scrollbar being active...
...
void ImGui::DebugNodeInputTextState(ImGuiInputTextState* state) {
    ...
    if (BeginChild("undopoints",
                   ImVec2(0.0f, GetTextLineHeight() * 15),
                   true))  // Visualize undo state
    {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel)) {
        ...
        // == ImGuiDir_Left), but we need to use SameLine() to setup baseline correctly. Might want
        // to refactor SameLine() to simplify this.
...
bool ImGui::ColorPicker4(const char* label,
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel) {
            ...
            if (ImTriangleContainsPoint(triangle_pa,
                ...
                if (!ImTriangleContainsPoint(triangle_pa,
                    ...
                    current_off_unrotated = ImTriangleClosestPoint(triangle_pa,
                                                                   ...
                                                                   current_off_unrotated);
                float uu, vv, ww;
                ImTriangleBarycentricCoords(triangle_pa,
                                            triangle_pb,
                                            triangle_pc,
                                            current_off_unrotated,
                                            uu,
        ...
        if (IsItemActive()) {
            ...
            ColorEditRestoreH(col, &H);  // Greatly reduces hue jitter and reset to 0 when hue ==
    ...
    ImU32 user_col32_striped_of_alpha = ColorConvertFloat4ToU32(ImVec4(
        R,
        G,
        B,
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
    ...
    IM_ASSERT(ImIsPowerOfTwo(
        flags & ImGuiColorEditFlags_DataTypeMask_));  // Check only 1 option is selected
    IM_ASSERT(ImIsPowerOfTwo(flags &
...
void ImGui::ColorTooltip(const char* text, const float* col, ImGuiColorEditFlags flags) {
    ...
    if (!BeginTooltipEx(ImGuiTooltipFlags_OverridePreviousTooltip, ImGuiWindowFlags_None)) return;
...
static bool IsRootOfOpenMenuSet() {
    ...
    // parent ID. This would however prevent the use of e.g. PuhsID() user code submitting menus.
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled) {
    ...
    if (g.MenusIdSubmittedThisFrame.contains(id)) {
        if (menu_is_open)
            menu_is_open =
                BeginPopupEx(id, window_flags);  // menu_is_open can be 'false' when the popup is
    ...
    // hover parent without always being a Child window) This is only done for items for the menu
    // set and not the full parent window.
                                   ...
                                   // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (g.HoveredWindow == window && child_menu_window != NULL) {
            ...
            float extra = ImClamp(ImFabs(ta.x - tb.x) * 0.30f,
                                  ref_unit * 0.5f,
        ...
        // slowly tends to hit same window, whereas moving away fast does not) But we also need to
        // not close the top-menu menu when moving over void. Perhaps we should extend the triangle
        // check to a larger polygon. (Remember to test this on BeginPopup("A")->BeginMenu("B")
        // sequence which behaves slightly differently as B isn't a Child of A and hovering isn't
        // shared.)
    ...
    if (want_open && !menu_is_open && g.OpenPopupStack.Size > g.BeginPopupStack.Size) {
        // Don't reopen/recycle same menu level in the same frame, first close the other menu and
        // yield for a frame.
        OpenPopup(label);
    } else if (want_open) {
        menu_is_open = true;
    ...
    if (menu_is_open) {
        ...
        SetNextWindowPos(popup_pos,
                         ImGuiCond_Always);  // Note: misleading: the value will serve as reference
                                             // for FindBestWindowPosForPopup(), not actual pos.
        PushStyleVar(ImGuiStyleVar_ChildRounding,
                     style.PopupRounding);  // First level will use _PopupRounding, subsequent will
                                            // use _ChildRounding
        menu_is_open =
            BeginPopupEx(id, window_flags);  // menu_is_open can be 'false' when the popup is
                                             // completely clipped (e.g. zero size display)
        ...
        if (menu_is_open) {
            ...
            // (This fixes using IsItemClicked() and IsItemHovered(), but IsItemHovered() also
            // relies on its support for ImGuiItemFlags_NoWindowHoverableCheck)
...
static void ImGui::TabBarLayout(ImGuiTabBar* tab_bar) {
    ...
    if (width_excess >= 1.0f && ((tab_bar->Flags & ImGuiTabBarFlags_FittingPolicyResizeDown) ||
...
void ImGui::TabItemBackground(ImDrawList* draw_list,
    ...
    if (g.Style.TabBorderSize > 0.0f) {
        ...
        draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding + 0.5f, y1 + rounding + 0.5f),
                                 rounding,
                                 6,
                                 9);
        draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding - 0.5f, y1 + rounding + 0.5f),
                                 rounding,
                                 9,
                                 12);
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list,
    ...
    const ImVec2 button_pos(ImMax(bb.Min.x, bb.Max.x - frame_padding.x * 2.0f - button_sz),
                            bb.Min.y);
    ...
    if (close_button_visible) {
        ...
        const ImRect bullet_bb(
            button_pos,
            button_pos + ImVec2(button_sz, button_sz) + g.Style.FramePadding * 2.0f);
