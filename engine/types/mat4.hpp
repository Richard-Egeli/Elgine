...
struct Mat4 {
    ...
    Vec3<float> Left() const {
        const glm::mat4 inv  = glm::inverse(matrix);
        const glm::vec3 left = glm::normalize(glm::vec3(inv[0]));
        return Vec3<float>(left.x, left.y, left.z);
    }
    ...
    Vec3<float> Translate(Vec3<float> pos) {
        matrix           = glm::translate(matrix, glm::vec3(pos.x, pos.y, pos.z));
        glm::vec3 newPos = glm::vec3(matrix[3]);

        return Vec3<float>(newPos.x, newPos.y, newPos.z);
    }

    Vec3<float> SetPosition(Vec3<float> pos) {
        Set(0, 3) = pos.x;
        Set(1, 3) = pos.y;
        Set(2, 3) = pos.z;
